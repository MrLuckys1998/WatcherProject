...
CLUSTERS = {
    "Cluster-1": {"url": "https://cluster1", "user": "admin", "pass": "temporal"},
    "Cluster-2": {"url": "https://cluster2", "user": "admin", "pass": "temporal"}
}
...
# Definición de una función: load_templates
def load_templates():
    return [f for f in os.listdir(TEMPLATE_DIR) if f.endswith(".py.j2")]

# Definición de una función: load_example
...
# Definición de una función: index
def index():
    ...
    error, preview, cat_result, watcher_list = None, None, None, None
        ...
        if request.form.get("list_watchers") == "1":
            ...
            try:
                ...
                try:
                    ...
                    error = f"Error en el formato JSON del payload del webhook: {e}"
                    logger.error(error)

                try:
                    elastic_payload_data = json.loads(elastic_payload) if elastic_payload.strip() else {}
                except json.JSONDecodeError as e:
                    elastic_payload_data = {}
                    error = f"Error en el formato JSON del payload de Elasticsearch: {e}"
                    logger.error(error)
